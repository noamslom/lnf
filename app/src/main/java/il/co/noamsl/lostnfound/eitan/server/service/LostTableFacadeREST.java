...
@Stateless
@Path("lost")
public class LostTableFacadeREST extends AbstractFacade<LostTable> {

    @PersistenceContext(unitName = "lf_serverPU")
    private EntityManager em;

    public LostTableFacadeREST() {
        super(LostTable.class);
    }

    /**
     * Adds a new Lost entry
    ...
    public Integer create(LostTable entity) {
    ...
    @Consumes(MediaType.APPLICATION_XML)
    @Produces(MediaType.TEXT_PLAIN)
    public Integer edit(LostTable entity) {
        ...
        LostTable l = em.find(LostTable.class, entity.getRecordid());
        l.update(entity);
    ...
    /**
     * Gets all relevant Lost records with name like the given name, 
    ...
    public List<LostTable> getLostItems(@QueryParam("name") String name,
        ...
        String baseQuery = "SELECT f FROM LostTable f WHERE 1=1";
        if (null != name) {
